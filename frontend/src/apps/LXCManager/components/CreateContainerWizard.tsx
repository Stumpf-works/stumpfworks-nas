...
const STEPS: StepConfig[] = [
  { id: 'general', title: 'General', description: 'Basic configuration', icon: Box },
  { id: 'resources', title: 'Resources', description: 'CPU & Memory', icon: Cpu },
...
export function CreateContainerWizard({ isOpen, onClose, onSuccess }: CreateContainerWizardProps) {
  const [currentStep, setCurrentStep] = useState<WizardStep>('general');
  const [formData, setFormData] = useState<ContainerCreateRequest & { password?: string; password_confirm?: string; ssh_key?: string }>({
    name: '',
    template: 'ubuntu',
    release: 'jammy',
    architecture: 'amd64',
  ...
  const validateStep = useCallback((step: WizardStep): boolean => {
    ...
    if (step === 'general') {
      ...
      if (formData.memory_limit && (formData.memory_limit < 128 || formData.memory_limit > 16384)) {
        errors.memory = 'Memory must be between 128 MB and 16 GB';
      }
      if (formData.cpu_limit && (formData.cpu_limit < 1 || formData.cpu_limit > 16)) {
        errors.cpu = 'CPU cores must be between 1 and 16';
      }
    } else if (step === 'network') {
      if (formData.network_mode === 'bridged' && !formData.bridge) {
        errors.bridge = 'Bridge selection is required for bridged mode';
      }
    }
...
// Step Components
interface StepProps {
  formData: ContainerCreateRequest & { password?: string; password_confirm?: string; ssh_key?: string };
...
function GeneralStep({ formData, setFormData, errors }: StepProps) {
  ...
  const templates = [
    { value: 'ubuntu', label: 'Ubuntu', description: 'Popular Linux distribution' },
    { value: 'debian', label: 'Debian', description: 'Stable and reliable' },
    { value: 'alpine', label: 'Alpine', description: 'Lightweight and secure' },
    { value: 'centos', label: 'CentOS', description: 'Enterprise-grade Linux' },
    { value: 'fedora', label: 'Fedora', description: 'Cutting-edge features' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Configure basic container settings and choose your operating system
        </p>
      </div>
          ...
          {templates.map((template) => (
            ...
            >
              <input
                ...
                className="sr-only"
              />
              <div className="font-medium text-gray-900 dark:text-white">{template.label}</div>
      ...
      <div className="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/10 dark:to-indigo-900/10 rounded-xl border border-blue-200 dark:border-blue-800">
        ...
        <div className="mt-3 flex items-start gap-2 text-xs text-gray-600 dark:text-gray-400">
          ...
          <div>
            Minimum 8 characters. Leave empty to create container without password (you can set it later).
          </div>
        </div>
      </div>
...
function ResourcesStep({ formData, setFormData }: StepProps) {
  const getMemoryRecommendation = (mb: number) => {
    ...
    if (mb < 2048) return '✓ Good - for typical workloads';
    if (mb < 4096) return '✓✓ Great - for demanding apps';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Allocate CPU and memory resources for your container
        </p>
      </div>
      ...
      <div className="p-6 bg-gray-50 dark:bg-macos-dark-50 rounded-xl">
        ...
        <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          <p className="text-sm text-blue-900 dark:text-blue-200">
            {getMemoryRecommendation(formData.memory_limit!)}
          </p>
        </div>
      </div>
      ...
      <div className="p-6 bg-gray-50 dark:bg-macos-dark-50 rounded-xl">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-2xl font-bold text-macos-blue">{formData.cpu_limit}</span>
            <span className="text-sm text-gray-600 dark:text-gray-400">
              {formData.cpu_limit === 1 ? 'Core' : 'Cores'}
            </span>
          </div>
        </div>
        ...
        <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          <p className="text-sm text-blue-900 dark:text-blue-200">
            {getCpuRecommendation(formData.cpu_limit!)}
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Choose how your container connects to the network
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <label
          className={`
            block p-6 border-2 rounded-xl cursor-pointer transition-all
            ${
              formData.network_mode === 'internal'
                ? 'border-macos-blue bg-macos-blue/5 dark:bg-macos-blue/10'
                : 'border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500'
            }
          `}
        >
          <div className="flex items-start gap-4">
            <input
              ...
              className="mt-1 w-4 h-4 text-macos-blue"
            />
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <Network className="w-5 h-5 text-macos-blue" />
                <span className="font-semibold text-gray-900 dark:text-white">
                  Internal Network (lxcbr0)
                </span>
              </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              {formData.network_mode === 'bridged' && (
                ...
                >
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Select Bridge <span className="text-red-500">*</span>
                  </label>
                  <select
                    value={formData.bridge || 'br0'}
                    onChange={(e) => setFormData({ ...formData, bridge: e.target.value })}
                    className={`
                      w-full px-4 py-2 bg-white dark:bg-macos-dark-50 border rounded-lg
                      focus:ring-2 focus:ring-macos-blue focus:border-transparent
                      text-gray-900 dark:text-white
                      ${errors.bridge ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'}
                    `}
                  >
                    {bridges.map((bridge) => (
                  ...
                  {errors.bridge && (
                    <p className="text-sm text-red-600 dark:text-red-400 mt-1">{errors.bridge}</p>
                  )}
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Container will start automatically when the system boots
            </p>
          </div>
        </label>
      </div>
  ...
  const sections = [
    {
      ...
      items: [
        ...
        { label: 'Release', value: formData.release || 'default' },
        { label: 'Architecture', value: formData.architecture },
      ],
    },
    {
      title: 'Security',
    ...
    },
    {
      ...
      items: [
        {
          label: 'Memory',
          value: `${formData.memory_limit} MB (${(formData.memory_limit! / 1024).toFixed(1)} GB)`,
        },
        { label: 'CPU Cores', value: formData.cpu_limit?.toString() || '1' },
      ],
    },
    {
      title: 'Network',
      ...
      items: [
        {
          label: 'Mode',
          value: formData.network_mode === 'internal' ? 'Internal (lxcbr0)' : 'Bridged',
        },
        ...(formData.network_mode === 'bridged'
          ? [{ label: 'Bridge', value: formData.bridge || 'br0' }]
          : []),
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Review your container configuration before creation
        </p>
      </div>
      ...
      <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
        <p className="text-sm text-blue-900 dark:text-blue-200">
          ✓ Ready to create container "{formData.name}"
        </p>
      </div>
    </div>
  );
