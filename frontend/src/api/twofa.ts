...
export const twofaApi = {
  ...
  getStatus: async (): Promise<TwoFAStatus> => {
    const response = await client.get('/2fa/status');
  ...
  setup: async (): Promise<TwoFASetupResponse> => {
    const response = await client.post('/2fa/setup');
    return response.data.data;
  },
  ...
  enable: async (code: string): Promise<void> => {
    await client.post('/2fa/enable', { code });
  },
  ...
  disable: async (code: string): Promise<void> => {
    await client.post('/2fa/disable', { code });
  },
  ...
  regenerateBackupCodes: async (code: string): Promise<string[]> => {
    const response = await client.post('/2fa/backup-codes/regenerate', {
      code,
    });
  ...
  loginWith2FA: async (req: TwoFALoginRequest): Promise<any> => {
    const response = await client.post('/auth/login/2fa', req);
    return response.data.data;
  },
};
