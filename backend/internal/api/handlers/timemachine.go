...
func GetTimeMachineConfig(w http.ResponseWriter, r *http.Request) {
	if timeMachineManager == nil {
		respondWithError(w, http.StatusServiceUnavailable, "Time Machine not initialized", nil)
		return
	}
...
func UpdateTimeMachineConfig(w http.ResponseWriter, r *http.Request) {
	...
	if err := timeMachineManager.UpdateConfig(&config); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to update configuration", err)
		return
	}
...
func EnableTimeMachine(w http.ResponseWriter, r *http.Request) {
	...
	if err := timeMachineManager.Enable(); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to enable Time Machine", err)
		return
	}
...
func DisableTimeMachine(w http.ResponseWriter, r *http.Request) {
	...
	if err := timeMachineManager.Disable(); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to disable Time Machine", err)
		return
	}
...
func ListTimeMachineDevices(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to list devices", err)
		return
	}
...
func GetTimeMachineDevice(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusBadRequest, "Invalid device ID", err)
		return
	}

	device, err := timeMachineManager.GetDevice(uint(id))
	if err != nil {
		respondWithError(w, http.StatusNotFound, "Device not found", err)
		return
	}
...
func CreateTimeMachineDevice(w http.ResponseWriter, r *http.Request) {
	...
	if device.DeviceName == "" {
		respondWithError(w, http.StatusBadRequest, "Device name is required", nil)
		return
	}

	if err := timeMachineManager.CreateDevice(&device); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to create device", err)
		return
	}
...
func UpdateTimeMachineDevice(w http.ResponseWriter, r *http.Request) {
	...
	if err := timeMachineManager.UpdateDevice(&device); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to update device", err)
		return
	}
...
func DeleteTimeMachineDevice(w http.ResponseWriter, r *http.Request) {
	...
	if err := timeMachineManager.DeleteDevice(uint(id)); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to delete device", err)
		return
	}
...
func UpdateTimeMachineDeviceUsage(w http.ResponseWriter, r *http.Request) {
	...
	if err := timeMachineManager.UpdateDeviceUsage(uint(id)); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to update device usage", err)
		return
	}

	device, err := timeMachineManager.GetDevice(uint(id))
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to get updated device", err)
		return
	}
...
func UpdateAllTimeMachineDeviceUsages(w http.ResponseWriter, r *http.Request) {
	...
	if err := timeMachineManager.UpdateAllDeviceUsages(); err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to update device usages", err)
		return
	}
